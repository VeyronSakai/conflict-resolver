...
describe('ConflictAnalyzer', () => {
  ...
  describe('findMatchingRule', () => {
    it('should find a matching rule based on file pattern', () => {
      ...
      const rules = [
        ...
        new ConflictRule('*.js', undefined, ResolutionStrategy.Ours)
      ]
    ...
    it('should find a matching rule with wildcard pattern', () => {
      ...
      const rules = [
        new ConflictRule('src/**/*.ts', undefined, ResolutionStrategy.Manual),
    ...
    it('should match rule with specific conflict type', () => {
      ...
      const rules = [
        new ConflictRule('*.ts', 'both-modified', ResolutionStrategy.Manual),
        new ConflictRule('*.ts', 'deleted-by-us', ResolutionStrategy.Ours)
      ]
  ...
  describe('determineStrategy', () => {
    ...
    it('should return Manual strategy when no rule matches', () => {
      const file = new ConflictedFile('unknown.xml', ConflictType.BothModified)
      const rules = [
        new ConflictRule('*.ts', undefined, ResolutionStrategy.Ours),
        new ConflictRule('*.json', undefined, ResolutionStrategy.Theirs)
      ]
